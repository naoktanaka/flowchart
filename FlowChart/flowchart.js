...
const createFrame = R.curry(
  (l, r, id, description) => `${id}${l}"${description}"${r}`
)
...
isElseOrElseIfOrEndIfProc = str =>
  isElseProc(str) || isEndIfProc(str) || isElseIfProc(str)
...
let chart = new Vue({
  ...
  methods: {
    ...
    toMermaidFormat(converted) {
      ...
      // 枠を連結（Trueルート）
      let trueObj = this.linkTrueRoute(converted, stopProc, exitLoop)
      mermaidObject.push(...trueObj)

      // 枠を連結（Elseルート）
      let elseObj = this.linkElseRoute(ifProc, converted)
      mermaidObject.push(...elseObj)

      // 枠を連結（exit loop）
      let exitObj = this.linkExitLoop(exitLoop, converted)
      mermaidObject.push(...exitObj)
      ...
      // ホバー
      this.procList.forEach((v, i) =>
        mermaidObject.push(`click ${i} callback "${v}"`)
      )
    ...
    linkTrueRoute(converted, stopProc, exitLoop) {
      let obj = []
      converted.forEach((v, id, a) => {
        const next = a[id + 1]
        const arrow = isIfOrElseIfProc(v) ? yesArrow : normalArrow
        if (!next) return // 最後の項目
        if (stopProc.includes(id) || exitLoop.includes(id) || isElseProc(v))
          return

        if (!isElseOrElseIfProc(next)) {
          obj.push(chainFrame(id, id + 1, arrow))
          return
        }
        ...
        for (let j = id + 2; j < converted.length - 1; j++) {
          ...
          if (isEndIfProc(next)) {
            ...
            // 対応するEndIfが見つかった
            obj.push(chainFrame(id, j, arrow))
          }
        }
      })
      return obj
    },
    linkElseRoute(ifProc, converted) {
      let obj = []
      ifProc.forEach((v, id) => {
        let to = 0
        let ifNestNode = 0
        for (let j = v + 1; j < converted.length - 1; j++) {
          ...
          if (isElseOrElseIfOrEndIfProc(next)) {
            ...
            // 次が親階層のelseの場合、更にElse
            to = isElseProc(next) ? j + 1 : j // elseは枠が無いため次の項目
            break
          }
        }
        obj.push(chainFrame(v, to, noArrow))
      })
      return obj
    },
    linkExitLoop(exitLoop, converted) {
      let obj = []
      exitLoop.forEach((v, i) => {
        ...
        obj.push(chainFrame(v, to, warpArrow))
      })
      return obj
    }
  }
